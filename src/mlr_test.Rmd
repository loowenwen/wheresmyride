...
# Crowd density preprocessing (once)
...
engineer_features <- function(lat, lon,
                              mrt_lrt_coords,
                              mrt_passVol,
                              mrtNames_crowdDensity,
                              busstops_passengerVolume,
                              bus_coords) {
...
### Building dataframe with previously predicted results
...
results_df <- postal_codes_df %>%
  pmap_dfr(function(postal_code, lat, lon) {
    tryCatch({
      result <- predict_accessibility_from_coords(
        postal_code, lat, lon,
        mrt_lrt_coords,
        mrt_passVol,
        mrtNames_crowdDensity,
...
```{r}
mlr_data <- results_df %>% drop_na()
glimpse(mlr_data)

mlr_model <- lm(score ~ mrt_crowd_score + mrt_passenger_volume + bus_passenger_volume + num_mrt_nearby, data = mlr_data)
summary(mlr_model)
```
